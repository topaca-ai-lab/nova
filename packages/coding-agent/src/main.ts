...
import { handleLocalCommand } from "./local-cli.js";
...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.NOVA_OFFLINE);
	if (offlineMode) {
	...
	if (await handleLocalCommand(args)) {
		return;
	}
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.NOVA_STARTUP_BENCHMARK);
