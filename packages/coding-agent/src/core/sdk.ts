...
function resolveLocalDeterministicRuntimeConfig(): LocalDeterministicRuntimeConfig | undefined {
	const localEnabled = Boolean(
		process.env.NOVA_LOCAL_MODE ||
	...
	const maxRepairAttemptsRaw = Number.parseInt(process.env.NOVA_LOCAL_MAX_REPAIR_ATTEMPTS || "", 10);
	const maxRepairAttempts =
		Number.isFinite(maxRepairAttemptsRaw) && maxRepairAttemptsRaw > 0 ? maxRepairAttemptsRaw : 2;
	const fallbackMode =
		process.env.NOVA_LOCAL_FALLBACK_MODE === "meta_escalate" ? "meta_escalate" : ("finish" as const);
	return {
		enabled: true,
		enforceSingleToolCallPerTurn: true,
		maxRepairAttempts,
		fallbackMode,
	};
}
...
function getOpenRouterAttributionHeaders(
	...
	return {
		"HTTP-Referer": "https:/-ai/nova-pods.dev",
