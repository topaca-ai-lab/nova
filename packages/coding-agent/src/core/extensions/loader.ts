...
function readPiManifest(packageJsonPath: string): PiManifest | null {
	try {
		...
		if (pkg.nova && typeof pkg.nova === "object") {
			return pkg.nova as PiManifest;
		}
		return null;
	} catch {
		return null;
...
function resolveExtensionEntries(dir: string): string[] | null {
	// Check for package.json with "nova" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "nova" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// Check for package.json with nova manifest or index.ts
