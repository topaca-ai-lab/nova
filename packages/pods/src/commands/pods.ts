...
export const listPods = () => {
	...
	if (podNames.length === 0) {
		console.log("No pods configured. Use 'nova-pods pods setup' to add a pod.");
		return;
	}
...
export const setupPod = async (
	...
	const vllmApiKey = process.env.NOVA_API_KEY;
	...
	if (!vllmApiKey) {
		console.error(chalk.red("ERROR: NOVA_API_KEY environment variable is required"));
		console.error("Set an API key: export NOVA_API_KEY=your_api_key_here");
		process.exit(1);
	}
	...
	console.log(chalk.cyan(`  nova-pods start <model> --name <name>`));
};
